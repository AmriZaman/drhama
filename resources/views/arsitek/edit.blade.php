...
<div class="main">
            ...
            <div class="panel">
                  ...
                  <form action="/wisata/{{$wisata->id}}/update2" method="POST" enctype="multipart/form-data">
                    {{csrf_field()}}
                  <label class="form-group{{ $errors->has('nama_wisata') ? ' has-error' : '' }}"> <span> Nama Wisata </span> </label>
									<input type="text" class="form-control" placeholder="Nama Wisata" name="nama_wisata" value="{{auth()->user()->wisata->nama_wisata}}">
									<br>
                  <label class="form-group{{ $errors->has('alamat_wisata') ? ' has-error' : '' }}"> <span> Alamat Wisata </span> </label>
									<input type="text" class="form-control" placeholder="Alamat Wisata" name="alamat_wisata" value="{{auth()->user()->wisata->alamat_wisata}}">
									<br>
                  <label class="form-group{{ $errors->has('jenis_wisata') ? ' has-error' : '' }}"> <span> Jenis Wisata </span> </label>
									<input type="text" class="form-control" placeholder="Jenis Wisata" name="jenis_wisata" value="{{auth()->user()->wisata->jenis_wisata}}">
									<br>
                  <label class="form-group{{ $errors->has('jam_operasional') ? ' has-error' : '' }}"> <span> Jam Operasional </span> </label>
									<input type="text" class="form-control" placeholder="Jam Operasional Wisata" name="jam_operasional" value="{{auth()->user()->wisata->jam_operasional}}">
									<br>
                  <label class="form-group{{ $errors->has('deskripsi_wisata') ? ' has-error' : '' }}"> <span> Deskripsi Wisata </span> </label>
									<textarea class="form-control" placeholder="Deskripsi Wisata" rows="4" name="deskripsi_wisata">{{auth()->user()->wisata->deskripsi_wisata}}</textarea>
									<br>
